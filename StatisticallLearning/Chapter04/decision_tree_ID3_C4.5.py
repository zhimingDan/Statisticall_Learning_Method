...
class DecisionTree:
    def __init__(self, data, labels, sort):
        self.data = data
        self.labels = labels
        self.sort = sort
        self.root = None
    ...
    # 生成树模型
    def create_tree(self, data, labels, sort):
        tree = {}
        # 判断是否到达树的末尾
        tree = {'over': False}
        # 如果当前结点的中的数据个数为0，或者当前结点中的标签只有一个种类,或者当前类别中都是相同的类别，那么此时也终止判断
        if len(data) == 0 | len(np.unique(labels)) == 1 | self.affirm_sort(labels):
            # 此时表示已经到达了树的构建的末尾
            tree['over'] = True
        ...
        if sort in ['ID3', 'C4.5']:
            ...
            for num_feature in range(num_features):
                ...
                unique_sorts, sort_counts = np.unique(data[:, num_features], return_counts=True)
                # 计算每个类别的条件熵
                for unique_sort in range(unique_sorts):
        ...
        if sort == 'ID3':
            ...
            for i, (gain, gain_ratio) in enumerate(zip(gains, gains_ratio)):
                ...
                if gain > mean_gain & gain_ratio > max_gain_ratio:
                    max_feature_index = i
    ...
    def use_decision_tree_predict(self, root, test_data):
        ...
        # 重新对输入的数据进行拼接
        test_data = np.hstack((test_data[:current_feature], test_data[current_feature + 1:]))
        # 根据当前测试数据的该最佳特征处的值，判断应该进行树的哪一边
        return self.use_decision_tree_predict(root['child'][test_data[current_feature]], test_data)
